...
static func get_character_list() -> Array:
	...
	for file in GDialog_Resources.listdir(GDialog_Resources.get_path('CHAR_DIR')):
		if '.json' in file:
			var data: Dictionary = GDialog_Resources.get_character_json(file)
...
static func get_timeline_list() -> Array:
	var timelines: Array = []
	for file in GDialog_Resources.listdir(GDialog_Resources.get_path('TIMELINE_DIR')):
		if '.json' in file: # TODO check for real .json because if .json is in the middle of the sentence it still thinks it is a timeline
			var data = GDialog_Resources.get_timeline_json(file)
...
static func get_theme_list() -> Array:
	var themes: Array = []
	for file in GDialog_Resources.listdir(GDialog_Resources.get_path('THEME_DIR')):
		if '.cfg' in file:
			var config = GDialog_Resources.get_theme_config(file)
			themes.append({
				'file': file,
...
static func get_default_definitions_list() -> Array:
	return GDialog_DefinitionsUtil.definitions_json_to_array(GDialog_Resources.get_default_definitions())
...
static func get_full_resource_folder_structure():
	return GDialog_Resources.get_resource_folder_structure()
...
static func set_folder_at_path(path: String, data:Dictionary):
	...
	GDialog_Resources.save_resource_folder_structure(new_data)
	return OK
	...
	if delete_files:
		for file in get_folder_at_path(folder_path)['files']:
			...
			match folder_path.split("/")[0]:
				'Timelines':
					GDialog_Resources.delete_timeline(file)
				'Characters':
					GDialog_Resources.delete_character(file)
				'Definitions':
					GDialog_Resources.delete_default_definition(file)
				'Themes':
					GDialog_Resources.delete_theme(file)
...
static func update_resource_folder_structure():
	var character_files = GDialog_Resources.listdir(GDialog_Resources.get_path('CHAR_DIR'))
	var timeline_files = GDialog_Resources.listdir(GDialog_Resources.get_path('TIMELINE_DIR')) 
	var theme_files = GDialog_Resources.listdir(GDialog_Resources.get_path('THEME_DIR'))
	var definition_files = get_default_definitions_dict().keys()
	
	var folder_structure = GDialog_Resources.get_resource_folder_structure()
	...
	GDialog_Resources.save_resource_folder_structure(folder_structure)
...
static func path_fixer_load(path):
	...
	match path:
		'res://addons/GDialog/Fonts/DefaultFont.tres':
			return load("res://addons/GDialog/Example Assets/Fonts/DefaultFont.tres")
		'res://addons/GDialog/Fonts/GlossaryFont.tres':
			return load('res://addons/GDialog/Example Assets/Fonts/GlossaryFont.tres')
		'res://addons/GDialog/Images/background/background-1.png':
			return load('res://addons/GDialog/Example Assets/backgrounds/background-1.png')
		'res://addons/GDialog/Images/background/background-2.png':
			return load('res://addons/GDialog/Example Assets/backgrounds/background-2.png')
...
static func resource_fixer():
	var update_index = GDialog_Resources.get_settings_config().get_value("updates", "updatenumber", 0)
	
	if update_index < 1:
		print("[D] Update NR. "+str(update_index)+" | Adds event ids. Don't worry about this.")
		for timeline_info in get_timeline_list():
			var timeline = GDialog_Resources.get_timeline_json(timeline_info['file'])
			...
			GDialog_Resources.set_timeline(timeline)
	
	GDialog_Resources.set_settings_value("updates", "updatenumber", 1)
